...
from .ImageReward import ImageReward, SaliencyImageReward
import torch
...
def load_saliency(name: str = "ImageReward-v1.0",
    ...
    model = SaliencyImageReward(device=device, med_config=med_config, reshape=reshape).to(device)
    msg = model.load_state_dict(state_dict, strict=False)
    print("checkpoint loaded")
    model.eval()
...
def load_to_train(name: str = "ImageReward-v1.0",
         device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None,
         med_config: str = None):
    """Load a ImageReward model
...
def load_to_train_saliency(name: str = "ImageReward-v1.0",
         device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", download_root: str = None,
         med_config: str = None, load = True):
    """Load a ImageReward model
    ...
    model = SaliencyImageReward(device=device, med_config=med_config).to(device)
    if load:
        msg = model.load_state_dict(state_dict, strict=False)
        print("checkpoint loaded")
    # model.eval()
